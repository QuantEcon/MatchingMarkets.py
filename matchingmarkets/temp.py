...
def basicNoBilateralCompatibility(agent1, agent2, cutoff):
    ...
    # print(agent1_has_quant, agent2_want_quant)
    if agent1_want == agent2_has and agent1_has != agent2_want and reasonableMatch(agent1, agent2, 0.1):
        return 1
    else:
        return 0
...
newsim = mm.simulation(runs= 1, time_per_run=1, max_agents=10000,
                        logAllData=True,
                       arrival_rate=100, success_prob=lambda: 1,
...
# Make sure matplotlib is _not_ inline for this
# newsim.graph(plot_time=5, period=1)

newsim.run()
print(newsim.stats())
print(len(newsim.record_matches))
...
print("unmatched agents total: " ,len(unmatched))
