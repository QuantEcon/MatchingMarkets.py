...
class Market:
    ...
    def __init__(self, arrival_rate=1, average_prob=lambda: 1,
                 max_agents=1000, graph=False, plots=False,
                 plot_time=0.5):
        """
    ...
    def update(self, metaAlgorithm=meta_always, algorithm=arbitraryMatch,
               neighborFct=rngDraw, discount=lambda: 0, metaParams=dict(),
               ...
               arrival_fct=lambda x: poisson.rvs(x), verbose=False,
               ):
        """
        ...
        if self.plots_on:
            ...
            if self.time != 0:
                ...
                nx.draw_networkx(self.Graph, pos=self.graph_pos,
                                 ...
                                 node_color=new_colors, node_size=200)
                plt.axis('off')
                plt.plot()
                ...
                for newAgent in self.Agents[-new_agents:]:
                    ...
                    if self.has_graph:
                        if oldAgent.match_fail_prob[newAgent.name] > 0:
                            self.Graph.add_edge(
                                    oldAgent, newAgent,
                                    weight=oldAgent.match_util[newAgent.name])
                if verbose:
                ...
                for otherAgent in [x for i, x
                        ...
                        if self.has_graph:
                            if newAgent.match_fail_prob[otherAgent.name] > 0:
                                self.Graph.add_edge(
                                    newAgent, otherAgent,
                                    weight=newAgent.match_util[otherAgent.name]
                                                    )
                if verbose:
        ...
        if verbose:
            print("\nPerforming matches ")
            print("Current Pool ", [a.name for a in self.Agents])
        matches = metaAlgorithm(self, match=algorithm, **metaParams)
        if verbose:
            print("\nMatches: ", matches)
        ...
        if selfMatch is False:
            ...
            for source in matchCopy.keys():
                if matchCopy[source] == source:
                    del matches[source]
                    if verbose:
                        print("Deleted selfMatch", source)
